...
mixin (
  ...
  public query ({ caller }) func listTrashedWarehouseItemEntries() : async [(Nat, Int)] {
    if (not AccessLib.isAdmin(accessRoles, caller)) { Runtime.trap("Only admin can view this"); };
    var result = List.empty<(Nat, Int)>();
    for ((id, ts) in warehouseItemsTrashed.entries()) { result.add((id, ts)); };
    result.toArray();
  };

  public query ({ caller }) func listTrashedStockMovementEntries() : async [(Nat, Int)] {
    if (not AccessLib.isAdmin(accessRoles, caller)) { Runtime.trap("Only admin can view this"); };
    var result = List.empty<(Nat, Int)>();
    for ((id, ts) in stockMovementsTrashed.entries()) { result.add((id, ts)); };
    result.toArray();
  };
