...
export function ProjectTemplatesPanel({ actor }: { actor: any }) {
  ...
  const [schedule, setSchedule] = useState<{ id: number; title: string; category: string; start: string; end: string; isoStart: string; isoEnd: string }[] | null>(null);
  const [buildSaved, setBuildSaved] = useState(false);
  const [buildSaveError, setBuildSaveError] = useState("");
  ...
  const generateSchedule = () => {
    ...
    const rows = editing[tpl.key].map((task) => {
      ...
      return { id: task.id, title: task.title, category: task.category, start: fmtDate(start), end: fmtDate(end), isoStart: toIso(start), isoEnd: toIso(end) };
    });
    setSchedule(rows);
    setBuildSaved(false);
    setBuildSaveError("");
  ...
  const saveAsTeamBuild = async () => {
    if (!schedule || !schedProject.trim()) { setBuildSaveError("Podaj numer projektu (np. BAS006)."); return; }
    setBuildSaveError("");
    try {
      await actor.createProjectBuild(
        schedProject.trim(),
        schedKey,
        schedStart,
        schedule.map((r) => ({
      ...
      setBuildSaveError("Zapis nieudany: " + (e?.message || String(e)));
    }
  };
        ...
        {schedule && (
          <div className="mobile-scroll-table overflow-auto">
            ...
            <div className="flex items-center gap-2 mt-2">
              <button
                onClick={saveAsTeamBuild}
                disabled={buildSaved}
                className="px-3 py-1.5 text-sm rounded bg-emerald-600 hover:bg-emerald-500 disabled:opacity-50 text-white"
              >
                {buildSaved ? "Zapisano — widoczne w postępie budowy" : "Zapisz jako aktywny build (widoczny dla zespołu)"}
              </button>
              {buildSaveError && <span className="text-xs text-red-500">{buildSaveError}</span>}
            </div>
          </div>
        )}
      </div>
