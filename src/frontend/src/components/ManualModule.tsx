...
const sections: Section[] = [
  ...
  },
  {
    ...
    title: "Dokumentacja",
    body: [
      "Wybór urządzenia i rozdziałów: u góry strony wybierasz urządzenie z rozwijanej listy (BAS/FNPT II/Trainer) — każde ma własny, niezależny zestaw rozdziałów. Rozdziały widać w panelu po lewej: przeciągnij za „⠿”, żeby zmienić kolejność, kliknij ✏ przy rozdziale żeby zmienić nazwę, ⬇ żeby pobrać, ✕ żeby usunąć. Checkbox przy rozdziale („Uwzględnij w podglądzie wydruku i eksporcie”) decyduje, czy dany rozdział wejdzie do PDF/Word przy eksporcie całości.",
      "Edytor instrukcji obsługi urządzeń — każde urządzenie ma własny zestaw rozdziałów, edytowanych jak dokument tekstowy (formatowanie, tabele, obrazy, sekcje, komentarze). Treść rozdziałów jest przechowywana na dysku Bartolini (OneDrive) — to jest wersja ostateczna; canister trzyma tylko metadane i opcjonalną kopię do wyszukiwania (patrz „Zmienne referencyjne” niżej).",
      "Tryb edycji („✏ Edytuj”) blokuje rozdział dla innych — blokada wygasa automatycznie po 30 sekundach braku aktywności, więc nie trzeba pamiętać o ręcznym zwalnianiu przy zamknięciu karty.",
      "„🖨 Podgląd wydruku” otwiera osobne, przeciągalne okno z podglądem realnych stron A4 (można je też otworzyć w zupełnie osobnym oknie przeglądarki — przydatne np. na drugim monitorze). Pokazuje dokładnie to, co wyjdzie na eksporcie PDF i Word — te trzy miejsca (edytor, podgląd, eksport) mają identyczny układ strony, marginesy i numerację. Jeśli coś źle wygląda w podglądzie, będzie źle wyglądać też w eksporcie.",
      "„🔗 Zmienne referencyjne” — narzędzie do masowej podmiany powtarzającego się tekstu (np. numeru seryjnego, wersji oprogramowania) we wszystkich rozdziałach naraz, zamiast ręcznego wyszukiwania. Dla każdej zmiennej wpisujesz „Aktualna wartość” (dokładny tekst widoczny teraz w dokumencie) i „Nowa wartość”, klikasz „Wstaw” — system znajduje wystąpienia i podmienia. Jeśli trafień jest więcej niż jedno, pokazuje listę do ręcznego zaznaczenia, które podmienić.",
      "Ważne: po udanej podmianie „Nowa wartość” automatycznie wskakuje do kolumny „Aktualna wartość” i staje się nowym punktem odniesienia — kolejna podmiana tej samej zmiennej działa już na nowej wartości, bez przepisywania niczego ręcznie. Samo to narzędzie (lista zmiennych) nie jest częścią dokumentu — nie jest drukowane ani widoczne w eksporcie.",
      "Wyszukiwanie zmiennych działa tylko na rozdziałach z włączonym checkboxem backupu (zakładka „Backup” w tym samym panelu) — to pojedyncza kopia treści trzymana on-chain wyłącznie do przeszukiwania, bez historii wersji. OneDrive pozostaje wersją ostateczną; backup trzeba włączyć per rozdział, żeby zmienne go widziały.",
      "„⚙️ Nagłówek/stopka” — wspólne ustawienia dla całej instrukcji urządzenia (nie per rozdział): osobne teksty lewo/środek/prawo dla stron nieparzystych i parzystych w nagłówku, tekst stopki, wysokość nagłówka/stopki w cm, rozmiar czcionki, obramowanie, opcja pominięcia strony tytułowej i numeracja stron (w Wordzie to prawdziwe, samoaktualizujące się pole numeracji).",
      "Import z Worda (.docx) rozpoznaje polskie style, spis treści z kropkowaniem, obrazy i tabele (łącznie ze scalonymi komórkami) — wklejanie przez Ctrl+V też zachowuje formatowanie.",
    ],
  },
  {
    id: "logbook",
    ...
    title: "Dziennik",
    body: [
      "Dziennik lotów/sesji symulatora — ma dwie zupełnie osobne strony: panel w tej aplikacji (dla administratora/zespołu) oraz publiczna strona „/dziennik” (dla instruktorów, bez logowania do głównego systemu).",
      "Publiczna strona /dziennik: logowanie e-mailem i 6-cyfrowym PIN-em (bez hasła do konta firmowego). Po zalogowaniu instruktor wybiera urządzenie z listy zarejestrowanych symulatorów, wypełnia formularz sesji (odpowiednik papierowego formularza FNPT II BAS FBXP) i podpisuje się palcem/rysikiem na ekranie — podpis jest wymagany, bez niego sesji nie da się zapisać.",
      "Czas trwania sesji liczy się automatycznie z podanych godzin start/koniec, a licznik nalotu urządzenia („H:MM”, bez ograniczenia liczby godzin) sumuje się sam na podstawie poprzednich wpisów — instruktor nie musi nic przeliczać ręcznie.",
      "Podpowiedzi nazwisk (autouzupełnianie) pojawiają się na podstawie zarejestrowanych instruktorów i wcześniejszej historii wpisów — przyspiesza wypełnianie przy powtarzających się osobach.",
      "Panel administracyjny w tej aplikacji (zakładka „Dziennik” w Adminie) ma dwie części: „Wpisy” — podgląd wszystkich zapisanych sesji ze wszystkich urządzeń, oraz „Instruktorzy” — zarządzanie listą osób uprawnionych do logowania na /dziennik: dodawanie nowego instruktora, reset PIN-u (gdy ktoś zapomni), aktywacja/dezaktywacja dostępu.",
      "Bezpieczeństwo logowania na /dziennik: PIN jest przechowywany w postaci zahaszowanej (nie da się go odczytać, tylko zresetować), próby logowania są ograniczane (blokada po zbyt wielu nieudanych próbach) — jeśli instruktor zgłasza „zablokowane konto”, sprawdź w Adminie status i w razie potrzeby zresetuj PIN.",
    ],
  },
  {
    id: "agent",
